...

{
	...
	function wpte_enquiry_form() { 
		...
		$package_detail_fields = array(
			...
			'package_label' => array(
				...
				'field_label'   => __( 'Roteiro:', 'wp-travel-engine' ),
		...
		?>
			...
			<div class="wte_enquiry_contact_form-wrap">
				...
				<form name="wte_enquiry_contact_form" action="#" method="post" id="wte_enquiry_contact_form" class="wte_enquiry_contact_form">
					...
					<h2 class="enquiry-form-title"><?php echo esc_html__( 'Deixe sua mensagem.', 'wp-travel-engine' ); ?></h2>
					...
					<input type="submit" class="enquiry-submit" name="enquiry_submit_button" id="enquiry_submit_button" value="<?php echo esc_attr__( 'Enviar', 'wp-travel-engine' ); ?>">
	...
	function wte_enquiry_send_mail() {
		...
		if ($formdata['trip_type'] == 1) {  
			...
			$subject               = 'Nova proposta de viagem';
			...
			$admin_email_template_content    = $mail_obj->get_email_template( 'enquiry_tailor', 'admin', true, $formdata );
			...
			{	
				...
				$result['message'] = __( "Sua mensagem foi enviada com sucesso.", 'wp-travel-engine' );		
			...
			{	
				...
				$result['message'] = __( "Desculpe, ocorreu um erro e não conseguimos enviar sua mensagem. Por favor, tente novamente mais tarde.","wp-travel-engine" );		
			...
			if( isset($_POST['query_confirmation']) && $_POST['query_confirmation']!= 'on' ) {
				...
				$result['message'] = __( "Confirmação falhou, por favor tente novamente.","wp-travel-engine" );		
			...
			if ( ! isset( $_POST['action'] ) || 'wte_enquiry_send_mail' !== $_POST['action'] ) {
				...
				wp_send_json_error( array( 'message' => __( 'Ação inválida', 'wp-travel-engine' ) ) );
			...
			if ( ! isset( $_POST ) || empty( $_POST ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'Nenhum dado recebido pelo formulário. Por favor, tente novamente.', 'wp-travel-engine' ) ) );
			...
			if ( ! $email || ! is_email( $email ) ) {
				...
				wp_send_json_error( array( 'message' => __( 'Por favor informe um email válido.', 'wp-travel-engine' ) ) );
			...
			if ( ! empty( $validation_check ) && false === $validation_check['status'] ) {
				...
				$result['message'] = 'Campo obrigatório. Preencha corretamente.';
