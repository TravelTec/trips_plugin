...
class WTE_Cart {
	...
	public function add( $trip_id, $trip_price = 0, $trip_price_partial = 0, $pax, $price_key = '', $attrs = array() ) {
		...
		$cart_item_id = $this->get_cart_item_id( isset($trip_id)?$trip_id:'', $price_key = '', isset($attrs['trip_date'])?$attrs['trip_date']:'' );
	...
	function get_total( $with_discount = true ) {
		...
		$get_total = array(
			...
			'total_partial'      => wp_travel_engine_get_formated_price( $cart_total_partial ),
		);
