	...
	*/
	function wp_travel_engine_confirmation_shortcodes_callback()
	{
		...
		<form method="post" id="wp-travel-engine-order-form" action="<?php echo esc_url( get_permalink( $wp_travel_engine_thankyou ) )?>">
			...
			if(isset($options['travelers_information']) && 'yes' === $options['travelers_information'] )
			{
				if (isset($_POST))
				{
					$error_found = FALSE;
						...
						foreach ($personal_options as $key => $value) { ?>
							<div class='wp-travel-engine-personal-details'>
								...
								switch ($key) {
									...
									<input type="<?php echo $value['type'];?>" name="wp_travel_engine_placeorder_setting[place_order][travelers][<?php echo esc_attr( $key );?>]" id="<?php echo esc_attr( $key );?>" <?php if( $value['required'] == '1' ) { echo 'required';   } ?>>
									<?php
									break;
