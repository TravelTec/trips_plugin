    ...
    <div class="wpte-tab-sub wpte-horizontal-tab">
        <form method="post" action="options.php">
            ...
            <div class="wpte-tab-wrap">
                <a href="javascript:void(0);" class="wpte-tab wte-addons current"><?php esc_html_e( 'WP Travel Engine - Extensões', 'wp-travel-engine' ); ?></a>
            </div>

            <div class="wpte-tab-content-wrap">
                <div class="wpte-tab-content wte-addons-content current">
                    <div class="wpte-title-wrap">
                        <h2 class="wpte-title"><?php esc_html_e( 'Chaves de licença', 'wp-travel-engine' ); ?></h2>
                        <div class="settings-note">
                            <?php esc_html_e( 'Todas as extensões premium instaladas e ativadas no seu site estão listadas abaixo. Você pode adicionar/editar suas chaves de licença para cada extensão individualmente.', 'wp-travel-engine' ) ?>
                        </div>
                    </div> <!-- .wpte-title-wrap -->
                    ...
                    <?php 
                        if ( sizeof( $addon_name ) == 0 ) {
                            echo '<h3 class="active-msg" style="color:#CA4A1F;">'.__('Nenhuma extensão premium adicionada!', 'wp-travel-engine').'</h3>';
                        }
                        ...
                        foreach ($addon_name as $key => $value) {
                            ...
                            $activation_message = sprintf( __( 'Informe sua chave de licença para a extensão %1$s.', 'wp-travel-engine' ), $addon_real_name );
                            $msg_color = '';

                            if ( $wte_fixed_departure_license && $wte_fixed_departure_status == 'valid' ) {
                                $activation_message = sprintf( __( 'Sua chave de licença para a extensão %1$s foi ativada.', 'wp-travel-engine' ), $addon_real_name );
                                $msg_color = 'style="color:#11b411"';
                            } elseif(  $wte_fixed_departure_license && $wte_fixed_departure_status != 'valid' ) {
                                $activation_message = isset( $_GET['wte_license_error_msg'] ) && $_GET['wte_addon_name'] === $value ? $_GET['wte_license_error_msg'] : sprintf( __( 'Sua chave de licença para essa extensão %1$s não foi ativada. Por favor, ative.', 'wp-travel-engine' ), $addon_real_name );
                            ...
                            <div class="wpte-floated <?php echo esc_attr( $active_class ); ?>">
                                ...
                                <div class="wpte-field wpte-password">
                                    ...
                                    <?php if( 'valid' == $wte_fixed_departure_status ) : ?>
                                        ...
                                        <?php esc_html_e( 'Ativada', 'wp-travel-engine' ); ?>
                                        </span>
                                    <?php endif; ?>
                                </div>
                                <?php if( $wte_fixed_departure_license ) { ?>
                                    ...
                                    <?php if( $wte_fixed_departure_status == 'valid' ) { ?>
                                        <input type="submit" class="wpte-btn wpte-btn-deactive deactivate-license" data-id="<?php echo $value; ?>" name="edd_license_deactivate" value="<?php echo 'Desativar licença'; ?>"/>
                                    <?php } else {  ?>
                                        <input type="submit" class="wpte-btn wpte-btn-active activate-license" data-id="<?php echo $value; ?>" name="edd_license_activate" value="<?php echo 'Ativar licença'; ?>"/>
                                    <?php } ?>
                                    </div>
                                <?php } ?>
                            </div>
                ...
                if ( sizeof( $addon_name ) != 0 ) {
                        ?>
                            <div class="wpte-field wpte-submit">
								<input id="submit" type="submit" name="submit" value="<?php echo esc_attr__( 'Salvar alterações', 'wp-travel-engine' ); ?>">
							</div>
                        <?php
                } else{
                    echo '<a target="_blank" href="https://wptravelengine.com/downloads/category/add-ons/" class="button button-primary">'.__('Adquira agora','wp-travel-engine').'</a>';
                }
            ?>
        </form>
    </div>
