...
$payment_status_arr = array(
	'completed'        => __( 'Completo', 'wp-travel-engine' ),
	'cancelled'        => __( 'Cancelado', 'wp-travel-engine' ),
	'pending'          => __( 'Pendente', 'wp-travel-engine' ),
	'partially-paid'   => __( 'Parcialmente pago', 'wp-travel-engine' ),
	'refunded'         => __( 'Recusado', 'wp-travel-engine' ),
	'abandoned'        => __( 'Abandonado', 'wp-travel-engine' ),
	'voucher-waiting'  => __( 'Aguardando voucher', 'wp-travel-engine' ),
	'voucher-received' => __( 'Voucher recebido', 'wp-travel-engine' ),
	'check-waiting'    => __( 'Aguardando confirmação', 'wp-travel-engine' ),
	'check-received'   => __( 'Confirmação recebida', 'wp-travel-engine' ),
);
?>
<div class="wpte-block wpte-col3">
	<div class="wpte-title-wrap">
		<h4 class="wpte-title"><?php _e( 'Pagamento', 'wp-travel-engine' ); ?></h4>
	...
	<div class="wpte-block-content">
		<ul class="wpte-list">
			<li>
				<b><?php _e( 'Status', 'wp-travel-engine' ); ?></b>
			...
			<li>
				<b><?php _e( 'Gateway', 'wp-travel-engine' ); ?></b>
	...
	<div class="wpte-block-content">
		...
		if ( ! empty( $additional_fields ) ) {
			...
			?>
			<h4 class="wpte-title"><?php echo $partial_label . ' ' . __( 'Detalhes', 'wp-travel-engine' ); ?></h4>
			<ul class="wpte-list">
				<?php
				...
				if ( isset( $booking_meta['place_order']['cost'] ) && ! empty( $booking_meta['place_order']['cost'] ) ) {
					?>
					<li>
						<b><?php echo $partial_label . ' ' . __( 'Valor pago', 'wp-travel-engine' ); ?></b>
						<span>
						<?php
						if ( isset( $booking_meta['place_order']['partial_cost'] ) && ! empty( $booking_meta['place_order']['partial_cost'] ) ) {
							echo wp_travel_engine_get_formated_price_with_currency_code_symbol( $booking_meta['place_order']['partial_cost'] );
						} elseif ( isset( $booking_meta['place_order']['cost'] ) && ! empty( $booking_meta['place_order']['cost'] ) ) {
							echo wp_travel_engine_get_formated_price_with_currency_code_symbol( $booking_meta['place_order']['cost'] );
						}
						?>
						</span>
					</li>
		...
		if ( ! empty( $remaining_payment_detail ) ) {
			?>
			<h4 class="wpte-title"><?php _e( 'Pagamento restante', 'wp-travel-engine' ); ?></h4>
			<ul class="wpte-list">
			<?php
			...
			if ( isset( $booking_meta['place_order']['partial_due'] ) && ! empty( $booking_meta['place_order']['partial_due'] ) ) {
				?>
					<li>
						...
						<?php
						if ( isset( $booking_meta['place_order']['partial_due'] ) && ! empty( $booking_meta['place_order']['partial_due'] ) ) {
							echo wp_travel_engine_get_formated_price_with_currency_code_symbol( $booking_meta['place_order']['partial_due'] );
						}
						?>
						</span>
					</li>
