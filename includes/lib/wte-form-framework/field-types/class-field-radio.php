		...
		if ( ! empty( $this->field['options'] ) ) {
			...
			foreach ( $options_arr as $key => $value ) {
				...
				if ( is_array( $this->field['default'] ) && count( $this->field['default'] ) > 0 ) {
					...
					$output .= sprintf( '<div class="wpte-bf-radio-wrap">
					<input type="radio" name="%s[]" value="%s" id="%s" %s %s %s>
					<label for="%s">
						%s
					</label>
				</div>', $this->field['name'], $key, $this->field['id'] . '_'. $index, $option_attributes, $checked, $validations, $this->field['id'] . '_'. $index, $value );
