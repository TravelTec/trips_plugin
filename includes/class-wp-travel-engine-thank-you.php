	...
	function wp_travel_engine_thank_you_shortcodes_callback() {
		...
		if ( isset( $_POST['wp-travel-engine-confirmation-submit'] ) && isset( $_SESSION['trip-id'] ) ) {
			if ( ! isset( $_POST['nonce'] ) || $_POST['nonce'] == '' || ! wp_verify_nonce( $_POST['nonce'], 'wp_travel_engine_final_confirmation_nonce' ) || ! isset( $_SESSION['trip-id'] ) ) {
				...
				return 'Desculpe, você pode não ter confirmado sua reserva. Preencha o formulário e confirme a sua reserva. Obrigado.';
			...
			if ( isset( $wp_travel_engine_settings['confirmation_msg'] ) && $wp_travel_engine_settings['confirmation_msg'] != '' ) {
				...
				$thankyou  = __( 'Obrigada por reservar essa viagem. Por favor, cheque seu e-mail para a confirmação.', 'wp-travel-engine' );
				$thankyou .= __( 'Abaixo os detalhes da sua reserva:', 'wp-travel-engine' );
				$thankyou .= '<br>';
			}
			echo wp_kses_post( $thankyou );
			?>
			<div class="thank-you-container">
				<h3 class="trip-details"><?php _e( 'Detalhes da viagem:', 'wp-travel-engine' ); ?></h3>
				<div class="detail-container">
					<div class="detail-item">
						<strong class="item-label"><?php _e( 'ID:', 'wp-travel-engine' ); ?></strong>
						<span class="value"><?php echo esc_attr( $_SESSION['trip-id'] ); ?></span>
					</div>
					<div class="detail-item">
						<strong class="item-label"><?php _e( 'Roteiro:', 'wp-travel-engine' ); ?></strong>
				...
				if ( isset( $_SESSION['due'] ) && $_SESSION['due'] != '' ) {
					?>
						<div class="detail-item">
							<strong class="item-label"><?php _e( 'Total pago:', 'wp-travel-engine' ); ?></strong>
						<?php
					...
					?>
						<div class="detail-item">
							<strong class="item-label"><?php _e( 'Total usto:', 'wp-travel-engine' ); ?></strong>
						<?php
						$cost = str_replace( ',', '', $_SESSION['trip-cost'] );
						?>
							<span class="value"><?php echo esc_attr( $currency . $obj->wp_travel_engine_price_format( $cost ) . ' ' . $code ); ?></span>
					...
					<?php } ?>
					<div class="detail-item">
						<strong class="item-label"><?php _e( 'Pagamento restante:', 'wp-travel-engine' ); ?></strong>
						<span class="value"><?php echo isset( $_SESSION['due'] ) ? esc_attr( $currency . $_SESSION['due'] . ' ' . $code ) : '-'; ?></span>
					</div>
					<div class="detail-item">
						<strong class="item-label"><?php _e( 'Início da viagem:', 'wp-travel-engine' ); ?></strong>
						<span class="value"><?php echo esc_attr( $_SESSION['trip-date'] ); ?></span>
					</div>
					<div class="detail-item">
						<strong class="item-label"><?php _e( 'Número de adultos:', 'wp-travel-engine' ); ?></strong>
						<span class="value"><?php echo esc_attr( $_SESSION['travelers'] ); ?></span>
					</div>
					<div class="detail-item">
						<strong class="item-label"><?php _e( 'Número de crianças', 'wp-travel-engine' ); ?></strong>
					...
					if ( isset( $_SESSION['extra_service'] ) && $_SESSION['extra_service'] != '' ) {
						...
						if ( $valid_exs ) :
							?>
							<div class="detail-item">
								<strong class="item-label"><?php _e( 'Serviços extras:', 'wp-travel-engine' ); ?></strong>
								<span class="value">
									<?php
									foreach ( $_SESSION['extra_service'] as $key => $value ) {
			...
			if ( ! isset( $_SESSION['trip-id'] ) ) {
				 $thank_page_msg   = __( 'Por favor, preencha o formulário e confirme sua reserva.', 'wp-travel-engine' );
				 $thank_page_error = apply_filters( 'wp_travel_engine_thankyou_page_error_msg', $thank_page_msg );
				 return 'Desculpe, você pode não ter confirmado sua reserva. Preencha o formulário e confirme a sua reserva. Obrigado.';
			}
