            ...
            <div class="details">
                    ...
                    foreach( $terms as $term ) {
                        ...
                        if ($my_query->have_posts()) { ?>
                            ...
                            <div class="wrap">
                                ...
                                <div class="category-main-wrap category-grid col-3 grid <?php echo esc_attr($term->term_id);?>" data-id="<?php echo $my_query->max_num_pages; ?>">
                                    <?php
                                        while ($my_query->have_posts()) : $my_query->the_post(); 
                                            $details = wte_get_trip_details( get_the_ID() );
                                            $details['j'] = $j;
                                            $details['destination_layout'] = true;
                                            wte_get_template( 'content-grid.php', $details );
                                            $j++;
                                        endwhile;
                                        if( $count > $default_posts_per_page )
                                        {
                                            echo "<div class='load-destination' data-query-vars='".json_encode( $my_query->query_vars )."' data-current-page='".$paged."' data-max-page='".$my_query->max_num_pages."'><span>".__("Load More Trips","wp-travel-engine")."</span></div>";
                                        }
                ...
                if ($others_query->have_posts()) { ?>
                    ...
                    <div class="wrap">
                        ...
                        <div class="category-main-wrap category-grid col-3 grid other" data-id="<?php echo $others_query->max_num_pages; ?>">
                            <?php
                                ...
                                {
                                    echo "<div class='load-destination' data-query-vars='".json_encode( $others_query->query_vars )."' data-current-page='".$paged."' data-max-page='".$others_query->max_num_pages."'><span>".__("Load More Trips","wp-travel-engine")."</span></div>";
                                }
                                ?>
                        </div>
                    </div>
