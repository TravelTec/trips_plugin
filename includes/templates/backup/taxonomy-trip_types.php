        ...
        <div class="wp-travel-engine-archive-outer-wrap">
            ...
            if($termchildren) {
                ...
                <div class="parent-desc" itemprop="description"><?php echo isset( $term_description ) ?  $term_description:'';?></div>
                    ...
                    {
                        echo "<div class='btn-loadmore' data-query-vars='".json_encode( $wte_trip_tax_post_qry->query_vars )."' data-current-page='".$paged."' data-max-page='".$wte_trip_tax_post_qry->max_num_pages."'><span>".__("Load More Trips","wp-travel-engine")."</span></div>";
                    }
                    ?>
                </div>
                <?php
                foreach ($termchildren as $child) {
                    ...
                    <div class="category-main-wrap category-grid col-3 grid <?php echo $term->term_id;?>" data-id="<?php echo $my_query->max_num_pages;?>">
                        <?php 
                        ...
                        if( $term->count > $default_posts_per_page )
                        {
                            echo "<div class='btn-loadmore' data-query-vars='".json_encode( $my_query->query_vars )."' data-current-page='".$paged."' data-max-page='".$my_query->max_num_pages."'><span>".__("Load More Trips","wp-travel-engine")."</span></div>";
                        }
                        ?>
                    </div>
                <?php
