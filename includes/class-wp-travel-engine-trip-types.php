 ...
 * @author
 */
class Wp_Travel_Engine_Trip_Types
{
	...
	*/
	function init()
	{
		add_shortcode( 'WP_TRAVEL_ENGINE_TRIP_TYPES', array( $this, 'wp_travel_engine_trip_types_shortcodes_callback' ) );
		add_filter( 'body_class', array( $this, 'add_trip_types_body_class' ) );
	}

	function add_trip_types_body_class( $classes ) {
		global $post;
		if ( is_object( $post ) ) {
			if ( has_shortcode( $post->post_content, 'WP_TRAVEL_ENGINE_TRIP_TYPES' ) ) {
				$classes[] = 'trip_types';
			}
		}
	...
	* @since 1.0
	*/
	function wp_travel_engine_trip_types_shortcodes_callback()
	{ 
		$retorno = '';
		...
		$retorno .= '<div id="wte-crumbs">'; 
              ...
              do_action('wp_travel_engine_breadcrumb_holder'); 
                    ...
                    $order = apply_filters('wpte_trip_types_terms_order','ASC');
                    $orderby = apply_filters('wpte_trip_types_terms_order_by','date');
                    $termchildren = get_terms($taxonomyName, array('orderby' => $orderby, 'order' => $order));
        $retorno .= '</div>
        <section class="featured-trip" id="featured_section" style="background: #fff;padding: 0;">
    <div class="container"> 
                            ...
                            <div class="holder">                            
                                <div class="img-holder">
                                    ...
                                    $image_id = get_term_meta ( $child->term_id, 'category-image-id', true );
                                      ...
                                      {
                                        $trip_types_thumb_size = apply_filters('wp_travel_engine_trip_types_img_size', 'trip_types-thumb-size');
                                        $image =  wp_get_attachment_image ( $image_id, $trip_types_thumb_size, false, array( 'itemprop' => 'image' ) );
                                        $retorno .= $image;
                                      }
                                      else{
