 ...
 * @author
 */
class Wp_Travel_Engine_Destinations
{
	...
	*/
	function init()
	{
		add_shortcode( 'WP_TRAVEL_ENGINE_DESTINATIONS', array( $this, 'wp_travel_engine_destinations_shortcodes_callback' ) );
		add_filter( 'body_class', array( $this, 'add_destinations_body_class' ) );
	}

	function add_destinations_body_class( $classes ) {
		global $post;
		if ( is_object( $post ) ) {
			if ( has_shortcode( $post->post_content, 'WP_TRAVEL_ENGINE_DESTINATIONS' ) ) {
	...
	* @since 1.0
	*/
	function wp_travel_engine_destinations_shortcodes_callback()
	{ 
		$retorno = '';
                                ...
                                <div class="img-holder">
                                    ...
                                    $image_id = get_term_meta ( $child->term_id, 'category-image-id', true );
                                      if(isset($image_id) && $image_id !='')
                                      {
                                        ...
                                        $image =  wp_get_attachment_image ( $image_id, $destination_thumb_size, false, array( 'itemprop' => 'image' ) );
                                        $retorno .= $image;
                                      }
                                      else{
