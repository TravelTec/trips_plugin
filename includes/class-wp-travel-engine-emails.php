...
class WP_Travel_Engine_Emails {
	...
	public function get_email_template( $type, $sent_to, $content_only = false, $data = false ) {
		...
		$strings = array(
			...
			'heading'         => __( 'Nova reserva', 'wp-travel-engine' ),

			'greeting'        => __( 'Caro Admin,', 'wp-travel-engine' ),

			'greeting_byline' => __( 'Voce tem uma nova reserva.', 'wp-travel-engine' ),
		...
		if ( $sent_to === 'customer' ) {
			...
			$strings = array(
				...
				'heading'         => __( 'Confirmação de reserva', 'wp-travel-engine' ),

				'greeting'        => __( '{name},', 'wp-travel-engine' ),

				'greeting_byline' => __( 'Sua solicitação de reserva foi concluída com sucesso.', 'wp-travel-engine' ),
	...
	public function send_booking_emails( $order_details, $booking_id ) {
		...
		$currency_code = ($wp_travel_engine_settings['currency_code'] == 'BRL' ? 'R$' : $wte_settings['currency_code']);
		...
		$fullname         = $order_details['place_order']['booking']['fullname'];

		$sitename         = get_bloginfo( 'name' );

		$city             = $order_details['place_order']['booking']['city'];

		$dados_passageiros = $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers'];

		$dados_travellers .= '<br>';
 

        for ($i=1; $i <= count($order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['fname']); $i++) { 
        	$nome_travellers[] = $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['fname'][$i];
        	$sobrenome_travellers[] = $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['lname'][$i];
        	$dob_travellers[] = $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['dob'][$i];
        	$title_travellers[] = $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['title'][$i];
 
			   		$data_nasc = explode("/", $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['dob'][$i]);
        	...
        	$dados_travellers .= '<p>
                    <span style="font-size: 13.7px;position: relative;top: 0px"><strong>Nome: </strong> '.$order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['fname'][$i].' '.$order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['lname'][$i].'</span>'; 
                    	$dados_travellers .= '<br><span style="font-size: 13.7px;position: relative;top: -6px"><strong>Tipo: </strong>'.esc_html( $tipo_pax ).'</span>
                    	<br><span style="font-size: 13.7px;position: relative;top: -12px"><strong>Nascimento: </strong>'.esc_html( $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['dob'][$i] ).'</span> <br><span style="font-size: 13.7px;position: relative;top: -18px"><strong>Gênero: </strong>'. $order_details['additional_fields']['wp_travel_engine_placeorder_setting']['place_order']['travelers']['title'][$i] .'</span>';
                $dados_travellers .= '</p>';
        }
		...
		$default_mail_tags = array(
			...
			'{tdate}'                     => implode("/", array_reverse(explode("-", $order_details['place_order']['datetime']))),
		...
		$subject_book = 'Nova reserva efetuada #' . $booking_id;
