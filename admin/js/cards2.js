...
function buscarCEP() {
    ...
    if (cep != "") {
        ...
        //Valida o formato do CEP. 
            ...
            $("#wp_travel_engine_booking_setting[place_order][booking][address]").val("...");
            $("#wp_travel_engine_booking_setting[place_order][booking][city]").val("...");
            $("#bairro").val("...");
            $("#estado").val("...");
            ...
            $.ajax(settings).done(function (response) {
                ...
                if (resposta["response"]["status"] == 200) {
                    ...
                    $("#wp_travel_engine_booking_setting[place_order][booking][address]").val(resposta["response"]["address"]);
                    $("#wp_travel_engine_booking_setting[place_order][booking][address]").focus();
                    $("#bairro").val(resposta["response"]["district"]);
                    $("#bairro").focus();
                    $("#wp_travel_engine_booking_setting[place_order][booking][city]").val(resposta["response"]["city"]);
                    $("#wp_travel_engine_booking_setting[place_order][booking][city]").focus();
