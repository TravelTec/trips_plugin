...
function wpte_tab_wpte_tarifario() {
    $ = jQuery;
    var file_frame;
    var allowed_filetype = ["image/jpeg", "image/png"];
    wte_tar_sortable();
    $(document).on("click", ".wpte-add-tarifario-img", function (e) {
        e.preventDefault();
        var $this = $(this);
        ...
        file_frame.on("select", function () {
            ...
            selection.map(function (attachment, i) {
                ...
                var file_html_content =
                    '<div class="wpte-tar-img"><input type="hidden" readonly name="wp_travel_engine_setting[tarifario][' +
                    index +
                    '][hotel][hotel_image]" value="' +
                    attachment.id +
                    '"><img src="' +
                    attachment.sizes.thumbnail.url +
                    '" alt=""><div class="wpte-tar-btns"><button class="wpte-change wpte-change-tar-img"></button><button class="wpte-delete wpte-delete-tar-img"></button></div></div>';

                $(file_html_content).insertBefore("#wpte-tar-img-upldr-btn");
                $("#wpte-tar-img-upldr-btn").attr('style', 'display:none');
            });
        });
        wte_tar_sortable();
        file_frame.open();
    ...
    $(document).on("click", ".wpte-change-tar-img", function (e) {
        e.preventDefault();
        var that = $(this);
        if (file_frame) file_frame.close();
        file_frame = wp.media.frames.file_frame = wp.media({
    ...
    $(document).on("click", ".wpte-add-hotel-tarifario", function (e) { 
        e.preventDefault();

        // Add itinerary.
        var iti_index =
            0 < jQuery(".wpte-tarifario-repeter").length
                ? jQuery(".wpte-tarifario-repeter").length + 1
                : 1;
        var template = wp.template("wpte-add-tar-row");

        jQuery(".wpte-remove-tar").remove();

        jQuery("#wpte-tarifario-repeter-holder").append(
            template({ key: iti_index })
        );
        ++iti_index;
    });  

    $(document).on("click", ".wpte-remove-hotel:last", function (e) {   
        e.preventDefault();
        ...
        jQuery(this).parents(".wpte-tarifario-repeter").remove();

        jQuery(".wpte-tarifario-repeter:last").append(
            '<button class="wpte-delete wpte-remove-hotel"></button>'
        );
    });
     ...
     */
    function wte_tar_sortable() {
        if ($(".wpte-tarifario").length) {
            $(".wpte-tarifario").sortable({
                opacity: 0.9,
                revert: true,
            });
        }
     ...
     */
    $(document).on("click", ".wpte-delete-tar-img", function (e) {
        e.preventDefault();
        $(this).parents(".wpte-tar-img").remove();
        $("#wpte-tar-img-upldr-btn").attr('style', 'display:block');
    });
    ...
    $(".wp-travel-engine-trip-video-tarifario-add-video").click(function (e) {
        e.preventDefault();
    ...
    function appendtemplate(ParsedURL, thumb) {
        var template = wp.template("wpte-trip-videotarifario-row");
        var rand = Math.floor(Math.random() * (999 - 10 + 1)) + 10;
        var vidthumb = thumb;

        $(".wp-travel-engine-trip-video-tarifario").append(
    ...
    $(document).on(
        "click",
        ".wp-travel-engine-trip-video-tarifario .remove-video",
        function (e) {
            e.preventDefault();
            var confirmation = confirm(WTE_UI.suretodel);
            if (!confirmation) {
...
function wpte_add_tarifas(){ 
    var e = window.event;
    e.preventDefault();

    // Add itinerary.
    var iti_index =
        0 < jQuery(".wpte-tarifa-repeter").length
            ? jQuery(".wpte-tarifa-repeter").length + 1
            : 1;
    var template = wp.template("wpte-add-tarifa-row");

    jQuery(".wpte-remove-tarifa").remove();
    jQuery("#lista_tarifas").attr("style", "display:none");
    jQuery(".display_none").attr("style", "display:none");

    jQuery("#wpte-tarifa-repeter-holder").append(
        template({ key: iti_index })
    );
...
function show_tarifario(id){
    var e = window.event;
    e.preventDefault();
    jQuery("#wpte-tarifa-div"+id).toggle(400);
    jQuery("#titulo-wpte-tarifa"+id).toggle(400);
    jQuery(".display_none").toggle(400);
    jQuery("#lista_tarifas").toggle(400);
}

function remove_tarifario(id){
    var e = window.event;
    e.preventDefault();
    ...
    jQuery("#wpte-tarifa-div"+id).remove();
    jQuery("#titulo-wpte-tarifa"+id).remove();
    jQuery("#linha_tarifa_"+id).attr('style', 'display: none');
    jQuery(".display_none").attr("style", "");
    //jQuery("#lista_tarifas").toggle(400);
}

function remove_tarifario_js(id){
    var e = window.event;
    e.preventDefault();
    ...
    jQuery("#wpte-tarifa-div"+id).remove();
    jQuery("#titulo-wpte-tarifa"+id).attr('style', 'display: none');
    jQuery("#linha_tarifa_"+id).attr('style', 'display: none');
    jQuery(".display_none").attr("style", "");
    jQuery("#lista_tarifas").attr('style', ' ');
}

function wpte_add_quartos_tarifario(value){ 
    var e = window.event;
    e.preventDefault();

        // Add itinerary.
        var iti_index =
            0 < jQuery(".wpte-quartos-repeter").length
                ? jQuery(".wpte-quartos-repeter").length + 1
                : 1;
        var template = wp.template("wpte-add-quartos-row"); 

        jQuery("#wpte-quartos-repeter-holder-"+value).append(
            '<div class="wpte-repeater-block wpte-quartos-repeter" style="border-top: 1px solid #eee"> <div class="wpte-quartos-fields"> <div class="wpte-field wpte-floated" style="margin-bottom: 0"> <h4 style="width: 100%;padding-top: 3px;">Apartamento <button class="wpte-delete wpte-remove-quarto" style="float: right;padding-top: 0px;"></button></h4> </div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Disponíveis </span> <input type="number" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][disponiveis]" value="" placeholder="Qtd. disponíveis" style="max-width: 85%;" class=""> </div><div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Pré reserva </span> <input type="number" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][pre_reserva]" value="" placeholder="Qtd. pré reserva" style="max-width: 85%;"> </div><div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Reservados </span> <input type="number" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][reservados]" value="" placeholder="Qtd. reservados" style="max-width: 85%;"> </div></div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated" style="max-width: 66%;"> <span class="wpte-tarifario-day">Categoria </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][categoria]" value="" placeholder="Categoria do quarto" style="max-width: 72%;"> </div><div class="wpte-floated" style="max-width: 34%;"> <span class="wpte-tarifario-day">Pax </span> <input type="number" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][pax]" value="" placeholder="Qtd. pax" style="max-width: 63%"> </div></div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated"> <span class="wpte-tarifario-day">Diárias</span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][diarias]" value="" placeholder="Qtd. diárias"> </div><div class="wpte-floated"> <span class="wpte-tarifario-day">Disponibilidade</span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][bloqueios]" value="" placeholder="Qtd. bloqueio" style="max-width: 52%"> </div></div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated"> <span class="wpte-tarifario-day">Início</span>  <input type="text" class="inicio" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][inicio]" value="" placeholder="Início"> </div><div class="wpte-floated"> <span class="wpte-tarifario-day">Término</span> <input type="text" class="termino" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][termino]" value="" placeholder="Término" style="max-width: 65%"> </div></div><div class="wpte-field wpte-floated"> <h4>Tarifas</h4> </div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated"> <span class="wpte-tarifario-day">Moeda</span> <select class="wpte-sublabel" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][moeda]" id="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][moeda]" style="max-width: 18%"> <option value="0">Moeda</option> <option value="R$" >R$</option> <option value="USD" >USD</option> <option value="U$" >U$</option> <option value="EUR" >EUR</option> <option value="CAN" >CAN</option> <option value="AUS" >AUS</option> <option value="NZ" >NZ</option> <option value="GBP" >GBP</option> </select> </div></div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Padrão </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][padrao]" value="" placeholder="0,00" style="max-width: 54%;" class=""  onKeyPress="return(moeda(this,\'.\',\',\',event))"> </div><div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Domingo </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][domingo]" value="" placeholder="0,00" style="max-width: 54%;"  onKeyPress="return(moeda(this,\'.\',\',\',event))"> </div><div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Segunda </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][segunda]" value="" placeholder="0,00" style="max-width: 54%;"  onKeyPress="return(moeda(this,\'.\',\',\',event))"> </div></div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Terça </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][terca]" value="" placeholder="0,00" style="max-width: 54%;margin-left: 11px;" class=""  onKeyPress="return(moeda(this,\'.\',\',\',event))"> </div><div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Quarta </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][quarta]" value="" placeholder="0,00" style="max-width: 54%;margin-left: 15px;"> </div><div class="wpte-floated" style="max-width: 33%;"  onKeyPress="return(moeda(this,\'.\',\',\',event))"> <span class="wpte-tarifario-day">Quinta </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][quinta]" value="" placeholder="0,00" style="max-width: 54%;margin-left: 13px;"  onKeyPress="return(moeda(this,\'.\',\',\',event))"> </div></div><div class="wpte-field wpte-multi-fields wpte-floated"> <div class="wpte-floated" style="max-width: 33%;"> <span class="wpte-tarifario-day">Sexta </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][sexta]" value="" placeholder="0,00" style="max-width: 54%;margin-left: 10px;" class=""> </div><div class="wpte-floated" style="max-width: 33%;"  onKeyPress="return(moeda(this,\'.\',\',\',event))"> <span class="wpte-tarifario-day">Sábado </span> <input type="text" name="wp_travel_engine_setting[tarifario]['+value+'][hotel][quarto]['+iti_index+'][sabado]" value="" placeholder="0,00" style="max-width: 54%;margin-left: 11px;"  onKeyPress="return(moeda(this,\'.\',\',\',event))"> </div></div></div></div>'
        );
        ++iti_index;
}

function remover_quarto(value){
    var e = window.event;
    e.preventDefault();
        ...
        jQuery("#wpte-quartos-repeter-"+value).remove(); 
}
/**
