                ...
                <div id="<?php echo esc_attr( $this->tab_attributes['id'] ); ?>" class="wpte-main-wrap <?php echo esc_attr( $this->tab_attributes['class'] ); ?>">
                    ...
                    <!-- Tabs Navigator -->
                        <div class="wpte-tab-wrap">
                            ...
                            ?>
                                <a data-next-tab="<?php echo esc_attr( json_encode( $next_tab ) ); ?>" data-post-id="<?php echo is_object( $post ) ? esc_attr( $post->ID ) : ''; ?>" data-callback="<?php echo esc_attr( $tab['callback_function'] ); ?>" href="javascript:void(0);" data-tab-details="<?php echo esc_attr( json_encode( array( 'content_path' => base64_encode( $tab['content_path'] ), 'content_key' => $tab['content_key'], 'tab_heading' => $tab['tab_heading'], 'tab_key' => $tab_key ) ) ); ?>" class="wpte-tab <?php echo esc_attr( $key ); ?> wpte-tab-nav <?php echo $tab['current'] ? ' current' : ''; ?><?php echo $tab['content_loaded'] ? ' content_loaded' : ''; ?>"><?php echo esc_html( $tab['tab_label'] ); ?></a>
                            <?php endforeach; ?>
                        </div>
