...
?>
    ...
    <div class="wpte-field wpte-text wpte-floated">
        ...
        <input type="text" name="wp_travel_engine_setting[trip_facts_title]" value="<?php echo esc_attr( $trip_facts_title ); ?>" placeholder="<?php esc_attr_e( 'Informe o título da seção', 'wp-travel-engine' ); ?>">
        <span class="wpte-tooltip"><?php esc_html_e( 'Informe o título que será exibido na aba de Fatos da Viagem.', 'wp-travel-engine' ) ?></span>
    </div>
    <div class="wpte-info-block">
        <b><?php esc_html_e( 'Nota:', 'wp-travel-engine' ); ?></b>
        <p><?php _e( sprintf('Você pode usar esse shortcode <b>%1$s</b> para exibir as informações desse roteiro em posts/páginas ou use esse código <b>%2$s</b> para exibir em templates.',$page_shortcode, $template_shortcode),'wp-travel-engine');?></p>
    </div>
    ...
    ?>
            <label class="wpte-field-label"><?php _e( 'Seleção da informação', 'wp-travel-engine' ); ?></label>
            <div class="wpte-floated">
                <select id="wte_global_trip_facts" data-nonce="<?php echo wp_create_nonce('trip-info-nonce') ?>" name="wte_global_trip_facts" data-placeholder="<?php esc_attr_e( 'Info Type&hellip;', 'wp-travel-engine' ); ?>">
                    <option value=""><?php _e( 'Selecione a informação', 'wp-travel-engine' ) ?></option>
                    <?php 
                ...
                <input type="button" class="button button-small add-info" value="<?php esc_attr_e( 'Adicionar informação', 'wp-travel-engine' ); ?>">
            </div>
            <span class="wpte-tooltip"><?php _e( 'Selecione o título da informação e clique em adicionar informação para adicionar conteúdo.', 'wp-travel-engine' ) ?></span>
    <?php 
        } else {
            esc_html_e( 'Informações padrões não encontradas. Por favor, primeiro adicione informações nas configurações globais.', 'wp-travel-engine' );
        } ?>
    </div>
                                ...
                                <?php 
                                    switch ($value) {
                                        case 'select': 
                                            ...
                                            ?>
                                            <select id="wp_travel_engine_setting[trip_facts][<?php echo $key;?>][<?php echo $key;?>]" name="wp_travel_engine_setting[trip_facts][<?php echo $key;?>][<?php echo $key;?>]" data-placeholder="<?php esc_attr_e( 'Selecione um tipo&hellip;', 'wp-travel-engine' ); ?>" class="wc-enhanced-select" >
                                            <option value=" "><?php _e( 'Selecione um tipo&hellip;', 'wp-travel-engine' ); ?></option>
                                            <?php
                                            foreach ( $options as $key => $val ) {
                                            if( isset($val) && $val !='' ){												
                                            $val = trim($val);
                                            echo '<option value="' .( !empty($val)?esc_attr( $val ):"Selecione um tipo")  . '" ' . selected( $selected_field, $val, false ) . '>' . esc_html( $val ) . '</option>';
                                            }
                                            }
                                            ?>
                                            </select>
                                        ...
                                        case 'duration' :
                                            ?>
                                                <input type="number" min="1" placeholder = "<?php _e('Número de dias','wp-travel-engine');?>" class="duration" id="wp_travel_engine_setting[trip_facts][<?php echo $key;?>][<?php echo $key;?>]" name="wp_travel_engine_setting[trip_facts][<?php echo $key;?>][<?php echo $key;?>]" value="<?php echo isset($wp_travel_engine_setting['trip_facts'][$key][$key]) ? esc_attr( $wp_travel_engine_setting['trip_facts'][$key][$key] ): '';?>"/>
                                            <?php
                                        break;
    ...
    <?php if ( $next_tab ) : ?>
        <div class="wpte-field wpte-submit">
            <input data-tab="facts" data-post-id="<?php echo esc_attr( $post_id ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'wpte-trip-tab-save-continue' ) ); ?>" data-next-tab="<?php echo esc_attr( $next_tab['callback_function'] ); ?>" class="wpte_save_continue_link" type="submit" name="wpte_trip_tabs_save_continue" value="<?php _e( 'Salvar e continuar', 'wp-travel-engine' ); ?>">
        </div>
    <?php endif;