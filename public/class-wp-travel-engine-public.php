...
class Wp_Travel_Engine_Public {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'pax_validation'  => __( 'Número de passageiros (%1$s) não é elegível para reserva. Por favor, selecione um número de passageiros entre %2$s e %3$s para esse roteiro.', 'wp-travel-engine' ),
				'bookingContinue' => _x( 'Continuar', 'Booking continue button Label', 'wp-travel-engine' ),
			)
		);
	...
	function wp_add_trip_cart() {
		...
		if ( array_key_exists( $_SESSION['cart_item'][ $_REQUEST['trip_id'] ], $_SESSION['cart_item'] ) ) {
			...
			$result['type']                                = 'success';
			$result['message']                             = __( 'Adicionado ao carrinho!', 'wp-travel-engine' );
		} else {
			$result['type']    = 'error';
			$result['message'] = __( 'Não foi possível adicionar ao carrinho.', 'wp-travel-engine' );
		}
	...
	function check_min_max_pax( $trip_id, $trip_price, $trip_price_partial, $pax, $price_key, $attrs ) {
		...
		if ( $min_max_enable ) {
			...
			if ( ( $travellers < $min_pax ) || ( $travellers > $max_pax ) ) {
				WTE()->notices->add( sprintf( __( 'Número de passageiros (%1$s) não é elegível para reserva. Por favor, selecione um número de passageiros entre %2$s e %3$s para esse roteiro.', 'wp-travel-engine' ), $travellers, $min_pax, $max_pax ), 'error' );
			}
		}
